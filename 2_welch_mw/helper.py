...
def plot_ab(p_A, p_B, n_A, n_B):
    ...
    sns.lineplot(x=xx, y=data_0, ax=ax)
    line = ax.get_lines()[-1]
    x,y = line.get_data()
    ...
    ax2 = ax.twinx()
    sns.lineplot(x=xx, y=data_1, ax=ax2, color='red')
...
def plot_alt(p_A, p_B, n_A, n_B):
    ...
    sns.lineplot(x=xx, y=data_1, ax=ax, color='red')
    line = ax.get_lines()[-1]
    x,y = line.get_data()
    mask_2 = x > norm.ppf(1-0.8, loc=d_1, scale=SE)
    x, y = x[mask_2], y[mask_2]
...
def plot_multiple_alt(d_0, d_1, d_2, SE_0, SE_1, SE_2):
    ...
    ax2 = ax.twinx()
    sns.lineplot(x=xx, y=data_1, ax=ax2, color='green')
    line = ax2.get_lines()[-1]
    x,y = line.get_data()
    mask = x > norm.ppf(1-0.05/2, loc=d_0, scale=SE_0)
    x, y = x[mask], y[mask]
    ...
    ax3 = ax.twinx()
    sns.lineplot(x=xx, y=data_2, ax=ax3, color='red')
